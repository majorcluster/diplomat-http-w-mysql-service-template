         ...
         (let [croissant {:name "Croissant"
           ...
           (testing "once breads are on db they are returned"
                    (let [id-1 (c.b/post croissant)
                      ...
                      (is (= (format "{\"breads\":[{\"id\":%s,\"name\":\"Croissant\",\"unit-grams\":200,\"price\":5.40}]}" id-1)
                             (:body resp)))
                      (is (= 200
                             (:status resp)))
                      (let [id-2 (c.b/post (assoc croissant :name "Ciabatta"))
                            resp (response-for service :get "/breads")]
                        (is (= (format "{\"breads\":[{\"id\":%s,\"name\":\"Croissant\",\"unit-grams\":200,\"price\":5.40},{\"id\":%s,\"name\":\"Ciabatta\",\"unit-grams\":200,\"price\":5.40}]}" id-1 id-2)
                               (:body resp)))
                        (is (= 200
                               (:status resp))))))))
         ...
         (let [croissant {:name "Croissant"
           ...
           (testing "once invalid id is sent, 404 is returned"
                    (let [resp (response-for service :get "/breads/473a4e4a-b4ec-44ea-a2be-ac67b9d9e205")]
                      (is (= 404
                             (:status resp))))
                    (let [resp (response-for service :get "/breads/aaay")]
                      (is (= 404
                             (:status resp)))))))
...
(deftest delete-bread
         ...
         (testing "when id is present, bread is deleted"
                  (let [croissant {:name "Croissant"
                    ...
                    (is (= 200
                           (:status resp)))
                    (is (= (merge {:id id} croissant) db-found-before))
